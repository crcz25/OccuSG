...
int dual_main(int argc, char ** argv) 
{
	...
	DUDE_ROS_WARN_STREAM() << "- Compute Dual-space Decomposition of "<<dude_param.Pfile<<" with tau "<<dude_param.concavity_tau<<" takes " << end_dude - start_dude
			<< " ms" << endl;
	...
	if (draw_options.showGL) {
		...
		gli::set2DMode(true);
		/////////////////////////////////////////////////////////////
		DUDE_ROS_WARN_STREAM() << "- Press ? to show GUI usage" << endl;
		gli::gliMainLoop();
	}
#endif
...
void loda_cuts(vector<c_polygon*>& finalPolygonPieces)
{
	...
	if(dude_param.decomposeIteratively){
		DUDE_ROS_DEBUG_STREAM()<<"Iteratively decomposing polygon"<<endl;
	...
	DUDE_ROS_DEBUG_STREAM()<<"Decomposition produced "<<finalPolygonPieces.size()<<" component(s)"<<endl;
...
void export_files(vector<c_polygon*>& finalPolygonPieces)
{
	...
	if (draw_options.savsPS) {
		...
		filename += ".ps";
		save2PS(filename);
		DUDE_ROS_DEBUG_STREAM() << "- Save " << filename << endl;
	}
#endif
}
...
void export_individual_files(vector<c_polygon*>& finalPolygonPieces)
{
	...
	for(int i = 0; i < psize; i++)
	{
		...
		ofile.close();
		DUDE_ROS_DEBUG_STREAM() << "- Saved: " << sname << endl;
	}
}
