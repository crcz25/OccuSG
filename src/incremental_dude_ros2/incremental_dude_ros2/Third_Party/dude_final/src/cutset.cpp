...
#if DEBUG
            DUDE_ROS_DEBUG_STREAM()<<"\t ("<<i->getV1()->getVID()<<","<<i->getV2()->getVID()<<") Angle="<<angle<<endl;
#endif
...
#if DEBUG
    if(diagonals.empty()){
        DUDE_ROS_WARN_STREAM()<<"! ERROR: no diagonals for building cutsets for PM["<<v->getVID()<<"]"<<endl;
    }
    DUDE_ROS_DEBUG_STREAM()<<"- PM["<<v->getVID()<<"] has "<<diagonals.size()<<" diagonals"<<endl;
#endif
...
#if DEBUG
    DUDE_ROS_DEBUG_STREAM()<<"- pos1="<<pos1<<endl;
    DUDE_ROS_DEBUG_STREAM()<<"- pos2="<<pos2<<endl;
#endif
...
#if DEBUG
    DUDE_ROS_DEBUG_STREAM()<<"- pos3="<<pos3<<endl;
#endif
...
#if DEBUG
            DUDE_ROS_DEBUG_STREAM()<<*i<<" [single pm]"<<endl;
#endif
...
#if DEBUG
        DUDE_ROS_DEBUG_STREAM()<<*shortest_cutset_resolve<<" [single tip]"<<endl;
#endif
    }
...
#if DEBUG
        DUDE_ROS_DEBUG_STREAM()<<*shortest_cutset_none_resolve<<" [single tip]"<<endl;
#endif
    }
...
#if DEBUG
            DUDE_ROS_DEBUG_STREAM()<<*i<<" [double pm pm]"<<endl;
#endif
...
#if DEBUG
            DUDE_ROS_DEBUG_STREAM()<<*i<<" [double tip pm] (1)"<<endl;
#endif
...
#if DEBUG
        DUDE_ROS_DEBUG_STREAM()<<*best_cutset<<" [double tip pm] (2)"<<endl;
#endif
...
#if DEBUG
        DUDE_ROS_DEBUG_STREAM()<<*best_cutset<<" [double tip tip]"<<endl;
#endif
        best_cutset->getV()->getExtra().cutsets.push_back(*best_cutset);
    }
}
...
//    {
//        check=true;
//        DUDE_ROS_DEBUG_STREAM()<<"check is true now"<<endl;
//    }
