...
AddPredecessorEdge(VID _v1id, VID _v0id, WEIGHT _weight) {
    ...
    if (IsVertex(_v1id,&cv1) && IsVertex(_v0id) ) {
         ...
         DUDE_ROS_DEBUG_STREAM() << endl << "AddPredecessorEdge: v1 " << _v1id << " and/or v0 " << _v0id << "not in graph" ;
         return 1;
     }
}
...
/*
    ...
    DUDE_ROS_DEBUG_STREAM()<<"\nTopological Sort results: "<<endl;
        DUDE_ROS_DEBUG_STREAM()<<tmp[i].first<<" ";
#endif
*/
    }
    return tps;            
...
FindPathBFS (VERTEX& _startV, VERTEX& _endV) const {
    ...
    if ( IsVertex(_startV,&cv1) && IsVertex(_endV,&cv2) ){
        ...
        DUDE_ROS_DEBUG_STREAM() << _startV << ", " << _endV << ") not in graph";
  ...
  if ( bfstree.IsVertex(_endVid,&cv1) && bfstree.IsVertex(_startVid,&cv2) ) {
     ...
     while ( !(path.begin()->first ==  cv2->data) ) {
        ...
        if ( bfstree.IsVertex(v2id,&cv1) ) {
           ...
           DUDE_ROS_DEBUG_STREAM() << "In FindPathBFS: hmm....\n";
        }
     }
  }
  return path;
...
DisplayVertex(VID _v1id) const {
    CVI v1;
    if ( IsVertex(_v1id,&v1) ) {
        DUDE_ROS_DEBUG_STREAM() << "vertex: id =" << setw(3) << _v1id; 
        DUDE_ROS_DEBUG_STREAM() << ", data = [" << v1->data;
        DUDE_ROS_DEBUG_STREAM() << "]" << endl;
    } else {
        DUDE_ROS_DEBUG_STREAM() << "vertex with id=" << _v1id << " not in graph.";
    }
}
...
ReadGraph(istream& _myistream) {
    ...
    if ( !strstr(tagstring,"GRAPHSTART") ) {
        DUDE_ROS_DEBUG_STREAM() << endl << "In ReadGraph: didn't read GRAPHSTART tag right";
        return;
    }
    ...
    for ( int i = 0; i < nVerts; i++){
      ...
      if ( !strstr(tagstring,"GRAPHSTOP") ) {
         DUDE_ROS_DEBUG_STREAM() << endl << "In ReadGraph: didn't read GRAPHSTOP tag right";
         return;
      }
}
template<class VERTEX, class WEIGHT>
...
DisplayCC ( VID _v1id) const {
    ...
    for (VI vi = ccverts.begin(); vi < ccverts.end(); vi++ ) {
        DUDE_ROS_DEBUG_STREAM() << *vi; 
        if (vi != ccverts.end() -1 ) DUDE_ROS_DEBUG_STREAM() << ", ";
    }
    DUDE_ROS_DEBUG_STREAM() << "}\n";
}
...
DisplayEdgesByCCVDataOnly() const {
    ...
    DUDE_ROS_DEBUG_STREAM() << endl << "Edges in each connected component (vertex data shown)";
    for (int cc=0; cc < ccedges.size(); cc++){
        DUDE_ROS_DEBUG_STREAM() << endl << "CC[" << cc << "]: ";
        for (int e=0; e < ccedges[cc].size(); e++){ 
            DUDE_ROS_DEBUG_STREAM() << " (" << ccedges[cc][e].first << "," << ccedges[cc][e].second << ")"; 
        }
    }
...
DisplayCCStats(int _maxCCprint) const {
    ...
    for (CCI vi = ccstats.begin(); vi != ccstats.end(); vi++) {
        DUDE_ROS_DEBUG_STREAM() << "\nCC[" << ccnum << "]: " << vi->first ; 
        DUDE_ROS_DEBUG_STREAM() << " (vid=" << vi->second << ")";
        ccnum++;
...
WtEdgeType<VERTEX,WEIGHT>:: 
DisplayEdge() const {
    DUDE_ROS_DEBUG_STREAM() << vertex2id << "(" << weight << ")";
} 
