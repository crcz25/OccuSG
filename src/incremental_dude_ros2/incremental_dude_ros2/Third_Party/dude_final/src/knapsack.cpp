...
#if DEBUG
        DUDE_ROS_DEBUG_STREAM()<<"capacity="<<n<<endl;
#endif
...
#if DEBUG
                            DUDE_ROS_DEBUG_STREAM()<<"- Remove cutset: id="<<all_cutsets[csetid].getV()->getVID()<<": "<<all_cutsets[csetid]<<endl;
#endif
                        }
                    }
                }
...
#if DEBUG
    uint size=cuts.size();
    DUDE_ROS_DEBUG_STREAM()<<"- final cuts:"<<endl;
    for(uint i=0;i<size;i++)
        DUDE_ROS_DEBUG_STREAM()<<"\t "<<cuts[i].getV1()->getVID()<<", "<<cuts[i].getV2()->getVID()<<endl;
#endif
...
#if DEBUG
            DUDE_ROS_DEBUG_STREAM()<<"m==0: add cutset"<<cset<<endl;
#endif
...
#if DEBUG
        DUDE_ROS_DEBUG_STREAM()<<"m=="<<m<<": skip cutset"<<cset<<endl;
#endif

    }
#if DEBUG
    else{
        DUDE_ROS_DEBUG_STREAM()<<"m=="<<m<<": add cutset"<<cset<<endl;
    }
#endif
...
void c_knapsack::cell::addCutset(c_cutset& cset)
{
    ...
    for(IT i=cutsets.begin();i!=cutsets.end();i++){
        if(i->conflict(cset)==false) new_cutset_list.push_back(*i);
            ...
            DUDE_ROS_DEBUG_STREAM()<<"conflict "<<*i<<" VS "<<cset<<endl;
#endif
