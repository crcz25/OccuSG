...
int _functor_BFS (GRAPH& _g , 
  ...
  while ( !q.empty() ) {
    ...
    if ( _g.IsVertex(v2id,&v2)) { 
      ...
      else DUDE_ROS_DEBUG_STREAM()<<"ERROR: colors in BFS functor"<<endl;
    }
      }
      _color[v1id] = BLACK;
      _f.finish_vertex(v1);
...
int _recursive_DFS(GRAPH& _g, visitor_base<GRAPH>& _f, map<VID,int>& _color, VID _v1id){
  ...
  while ( ei != v1->edgelist.end() ) {
    ...
    _f.ft_edge(v1,ei);
      ...
      else DUDE_ROS_DEBUG_STREAM()<<"ERROR: colors in DFS functor"<<endl;
...
int _functor_BFS_EQ (GRAPH& _g , 
  ...
  while ( !q.empty() ) {
    ...
    if ( _g.IsVertex(v2id,&v2)) { 
      ...
      else DUDE_ROS_DEBUG_STREAM()<<"ERROR: BFS EQ colors"<<endl;
...
int _recursive_DFS_EQ(GRAPH& _g, visitor_base<GRAPH>& _f, map<VID,int>& _color, VID _v1id){
  ...
  while ( ei != v1->edgelist.end() ) {
    ...
    if(_f.ft_edge(v1,ei) == EARLY_QUIT) return EARLY_QUIT;
      ...
      else DUDE_ROS_DEBUG_STREAM()<<"ERROR: DFS EQ colors"<<endl;
    } else {
      //DUDE_ROS_DEBUG_STREAM() << "\nIn GraphDFS: vid=" << v2id << " not in graph";
    }
    ei++;
...
public:
    ...
    void print() const
    {
        DUDE_ROS_DEBUG_STREAM()<<"Vertex : "<<vid
            <<"  Prev Vertex : "<<predvid
            <<"  Distance : "<<dist
...
#endif
{
    ...
    {
        //DUDE_ROS_DEBUG_STREAM()<<"Both d1 and d2 are not valid"<<endl;
        //The returned value does not make much sense
        return ( d1.dist > d2.dist );
    }
    else if(!d1.valid())
    {
        //DUDE_ROS_DEBUG_STREAM()<<"D1 not valid"<<endl;
        //So d1.dist > d2.dist
        return true;
    }
    else if(!d2.valid())
    {
        //DUDE_ROS_DEBUG_STREAM()<<"D2 is not valid"<<endl;
        return false;
    }
    else
    {
        //DUDE_ROS_DEBUG_STREAM()<<"Both are valid"<<endl;     
        return d1.dist > d2.dist;
    }
...
int FindPathDijkstra (GRAPH& _g, VID _v1id, VID _v2id,
  ...
  if(!_g.IsVertex(_v1id,&cvi)) {
    DUDE_ROS_DEBUG_STREAM()<<"ERROR while trying to retrieve the CVI in FindPathDijkstra"<<endl;
    return -1;
  }
...
int dfs_pred(GRAPH& _g,map<VID,int>& _color,vector<vector<VID> >& _scc,VID _start, int scc_no ){
  ...
  if(!_g.IsVertex(_start,&vi)){
    DUDE_ROS_DEBUG_STREAM()<<"ERROR:: inside dfs_pred"<<endl;
    return ERROR;
  }
...
int scc(GRAPH& _g, vector<vector<VID> >& _scc){
  ...
  DFS_functor(_g ,_vis);
  //for(i=0;i<ft.size();i++)
  //  DUDE_ROS_DEBUG_STREAM()<<"["<<ft[i].first<<" "<<ft[i].second<<"]"<<endl;
  _g.SetPredecessors();
