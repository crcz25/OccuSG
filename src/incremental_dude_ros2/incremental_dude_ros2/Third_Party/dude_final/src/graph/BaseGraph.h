...
public:
  ...
  COLOR SetColorOut(int _tr_id,COLOR _color){
    ...
    //  DUDE_ROS_DEBUG_STREAM()<<"somebody touching"<<vid<<": to set the color "<<_color<<":"<<stapl::get_thread_id()<<endl;
    //}
  ...
  void SetDependencies(int _tr_id){
    ...
    //if(vid == 4994 || vid== 5016 || vid== 5038 || vid==5060){
    //  DUDE_ROS_DEBUG_STREAM()<<"INSIDE::"<<_tr_id<<":"<<vid<<":"<< predecessors.size()<<":"<< edgelist.size()<<endl;
    //}
  }
  ...
  int ByteSize(){
    ...
    //DUDE_ROS_DEBUG_STREAM()<<"EDGELISTSIZE:"<<sizeof(edgelist[0])<<":"<<sizeof(float)<<endl;
...
public:
   ...
   inline int setVertIDs(VID _v){
     if(_v < vertIDs) {
       DUDE_ROS_WARNF("ERROR: You are trying to set as max vid ");
       DUDE_ROS_WARNF("something smaller than current one\n");
 ...
 public:
  ...
  //===========================
    ...
    void RemoveFromLocalIDMap(VID _gid){ 
      //DUDE_ROS_WARNF("vertex %d erased from LocalIDMap\n",_gid);
    ...
    DUDE_ROS_DEBUG_STREAM()<<"VID : "<<it->first<<"  with Local ID: "<<it->second<<endl;
      }
    }
#endif
...
DeleteVertices(vector<VID>& _vv) {
  ...
  DUDE_ROS_DEBUG_STREAM()<<"THIS METHOD SHOULD not BE CALLED"<<endl<<endl;
  VI v1;
...
DisplayGraph() const {
    ...
    for (vi = this->v.begin(), i=0; vi < this->v.end(); vi++, i++) {
      DUDE_ROS_DEBUG_STREAM() << setw(3) << i << ": ";
...
#ifndef QUIETGRAPH
    DUDE_ROS_DEBUG_STREAM() << endl << "AddEdge: v1 " << _v1id << " and/or v2 " << _v2id << "not in graph" ;
#endif
    return ERROR;
    }
}
...
#ifdef DEBUG2
    //DUDE_ROS_DEBUG_STREAM()<<"using local index"<<endl;
#endif
...
my_find_VID_eqc(const VID _vid) const {
    ...
    map<VID,int>::const_iterator mit = gid_localid_map.find(_vid);
    //DUDE_ROS_DEBUG_STREAM()<<"--- "<<_vid<<" "<<mit->second<<" "<<v.size()<<endl;
    if(mit ==  gid_localid_map.end()) return v.end();
    //startvi = v.begin() + mit->second;
    if ( v.size() > mit->second ) {
        startvi = v.begin() + mit->second;
...
DeleteEdgeId(int _edgeid) {
  ...
  DUDE_ROS_DEBUG_STREAM()<<"BaseGraph.h::DeleteEdgeId Inexistent edge"<<endl;
  return ERROR;
}
#endif
...
IsEdgeId(int _edgeid, EI* _ei)  {
  ...
  DUDE_ROS_DEBUG_STREAM()<<"BaseGraph.h::IsEdgeID Inexistent edge"<<endl;
  return false;
}
...
DisplayEdgelist(int wt) const {
    DUDE_ROS_DEBUG_STREAM() << "id =" << setw(3) << vid << ", data = ["; 
    DUDE_ROS_DEBUG_STREAM() << data;
    DUDE_ROS_DEBUG_STREAM() << "], edges={";
    {for (CEI ei = edgelist.begin(); ei < edgelist.end(); ei++) {
        ei->DisplayEdge(wt);
        if (ei != edgelist.end() - 1) DUDE_ROS_DEBUG_STREAM() << ", ";
    }}
    DUDE_ROS_DEBUG_STREAM() << "} Predecessors={";

    {for (CEI ei = predecessors.begin(); ei < predecessors.end(); ei++) {
        ei->DisplayEdge(wt);
        if (ei != predecessors.end() - 1) DUDE_ROS_DEBUG_STREAM() << ", ";
    }}
    DUDE_ROS_DEBUG_STREAM() << "} \n";
}
...
DisplayEdge(int wt) const {
  DUDE_ROS_DEBUG_STREAM() << vertex2id;
  if (wt==1) DUDE_ROS_DEBUG_STREAM() << "(" << weight << ")";
} 
