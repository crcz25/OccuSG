...
bool readPoly(const string& name, c_polygon& poly) {
	...
	if (!fin.good()) {
		DUDE_ROS_WARN_STREAM() << "! Error: Cannot open file (" << name << ")" << endl;
		return false;
	}
...
bool parseArg(int argc, char ** argv) 
{
	...
	for (c_polygon::iterator pit = getP().begin(); pit != getP().end(); ++pit) {
		...
		if (abs(tply.getArea()) < 0.01 * outRadius * outRadius) {
			...
			//DUDE_ROS_DEBUG_STREAM()<<"ignored"<<endl;
		}
	}
...
{
	int offset = 15;
	DUDE_ROS_WARN_STREAM() << "Usage: " << name << " [options] P.poly" // Q.poly \n"
			<< "\noptions:\n\n";

#if GL_RENDERING
	DUDE_ROS_WARN_STREAM() << left << setw(offset) << "-g" << " disable openGL visualization\n";
#endif

	DUDE_ROS_WARN_STREAM() << left << setw(offset) << "-o file"
			<< " dump decompoisition to file\n";

	//DUDE_ROS_WARN_STREAM() << left << setw(offset) << "-j" << " convert to json file\n";

	DUDE_ROS_WARN_STREAM() << left << setw(offset) << "-k [value]"
			<< " create k-number of decomposition\n";
	DUDE_ROS_WARN_STREAM() << left << setw(offset) << "-tau [value]" << " tolerable concavity\n";
	
	//DUDE_ROS_WARN_STREAM() << left << setw(offset) << "-com [file.txt]"
	//		<< " to compare the segmenation with a segmentation from file\n";
	
	DUDE_ROS_WARN_STREAM() << left << setw(offset) << "-ite"
			<< " decompse once\n";

	DUDE_ROS_WARN_STREAM() << left << setw(offset) << "-ske"
		<< " build skeleton after decomposition\n";

#if PS_RENDERING
	DUDE_ROS_WARN_STREAM() << left << setw(offset) << "-ps" << " saves to ps file\n";
	DUDE_ROS_WARN_STREAM() << left << setw(offset) << "-pske"
		<< " save skeleton ps file\n";
	DUDE_ROS_WARN_STREAM() << left << setw(offset) << "-pp" << " saves polygon pieces to ps file\n";
#endif

	DUDE_ROS_WARN_STREAM()<<left <<setw(offset)<<"-polys"
			<<" export decomposed polygons into poly files\n";


	DUDE_ROS_WARN_STREAM() << left << setw(offset) << "-svg"
		<< " export decomposed polygons into svg files\n";


	DUDE_ROS_WARN_STREAM() << flush;
}
...
void printGUIUsage() 
{
	...
	int offset = 15;
	DUDE_ROS_WARN_STREAM() << "GUI Usage:\n";

	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "p:"
			<< "show/hide P, the rotating polygon\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "n:" << "show/hide normal direction \n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "r:" << "reset everything\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "9:" << "on/off dumping pdf images\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "0:" << "on/off dumping ppm images\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "c:" << "show/hide concavities\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "d:" << "show/hide decomposition\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "b:" << "show/hide bridges\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "w:"
			<< "show/hide potential cuts/cutsets\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "G:" << "show/hide simplified polygon\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "P:" << "print to a PS file\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "5:" << "zoom in\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "6:" << "zoom out\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "C:"
			<< "show all potential cuts one by one,increase the index by pressing I\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "S:"
			<< "show all simplified polygons one by one,increase the index by pressing I\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "B:"
			<< "show all bridges one by one,increase the index by pressing I\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "I:"
			<< "increase the index of showing elements one by one\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "Y:"
			<< "show all the plys generated during the process one by one,\n"
			<< left << setw(offset) << " "
			<< "increase the index by pressing I\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "R:"
			<< "show all the generated rest plys excluding child pockets one by one,\n"
			<< left << setw(offset) << " "
			<< "increase the index by pressing I\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "H:"
			<< "show all the generated parent pocket and its children pockets one by one, \n"
			<< left << setw(offset) << " "
			<< "increase the index by pressing I\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "Q:" << "show cutted polygon pieces\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "U:" << "show all user cuts\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "L:"
			<< "show all user cuts in clustering.\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "u:" << "show rep user cuts\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "E:" << "show skeleton\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "t:" << "show cuts to compare with\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "arrows:" << "translate\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "ctrl-click:"
			<< "select a concavity vertex\n";
	DUDE_ROS_DEBUG_STREAM() << left << setw(offset) << "?:"
...
void DuDe() 
{
	...
	DUDE_ROS_DEBUG_STREAM() << "- Dual-space Decomposition with tau=" << dude_param.concavity_tau << endl;
	...
	//DUDE_ROS_DEBUG_STREAM()<<"- Compute Dual-space Decomposition takes "<<end_dude-start_dude<<" ms"<<endl;
	//glutPostRedisplay();
}
...
void Display(void) {
	...
	if (draw_options.saveImg || draw_options.savePDF) {
		...
		DUDE_ROS_WARN_STREAM() << "- Save Image : " << filename << endl;
	}
}
...
void Keyboard(unsigned char key, int x, int y) {
	...
	case 'P': //print to a PS file
		...
		save2PS(PSfilename);
		DUDE_ROS_DEBUG_STREAM() << "- Save " << PSfilename << endl;
		break;
	case 'r':
		resetCamera();
		break;
	...
	case '?':
		if (draw_decoration.g_selected_PM == NULL)
			...
			DUDE_ROS_DEBUG_STREAM() << "PM[" << draw_decoration.g_selected_PM->getVID() << "] has " << cset_size
					<< " cutsets, concavity="
					<< draw_decoration.g_selected_PM->getExtra().concavity << endl;
			for (uint j = 0; j < cset_size; j++) {
				DUDE_ROS_DEBUG_STREAM() << csets[j] << endl;
			}
			DUDE_ROS_DEBUG_STREAM() << "\t dihedral Pre Vid="
