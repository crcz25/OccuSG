...
ply_vertex * c_ply::addVertex( double x, double y, bool remove_duplicate )
{
    ...
    if(tail!=NULL){
        if(tail->getPos()==pt && remove_duplicate){
            DUDE_ROS_DEBUG_STREAM()<<"duplicated vertex"<<endl;
...
bool c_ply::doInit()
{
    ...
    if(this->area<0 && type==POUT){
       DUDE_ROS_WARN_STREAM()<<"! Warning: polygon type is POUT but has negative area. Reverse the vertex ordering."<<endl;
       reverse();

       b = false;
    }
    else if(this->area>0 && type==PIN){
       DUDE_ROS_WARN_STREAM()<<"! Warning: polygon type is PIN but has positive area. Reverse the vertex ordering."<<endl;
       reverse();
...
list<c_polygon> c_polygon::split()
{
    ...
    for(iterator i=begin();i!=end();i++){
        if(i->getType()==c_ply::POUT){
            ...
            if(sub.empty()){
                DUDE_ROS_WARN_STREAM()<<"! Error: Invalid polygon type: Holes are defined without external boundary."<<endl;
