...
bool c_cdt::build(c_dude& ap, c_polygon& sp, c_polygon& initPolygon)
{
    ...
    {
		//DUDE_ROS_DEBUG_STREAM()<<"the polygon already has some cuts before delaunay triangulation"<<endl;
    ...
    if(!cdt.is_valid()){ DUDE_ROS_WARN_STREAM()<<"! Warning: GDT failed"<<endl; return false; }
