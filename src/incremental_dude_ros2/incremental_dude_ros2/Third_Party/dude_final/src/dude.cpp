...
//     if(P[i]->getExtra().concavity_bpc) DUDE_ROS_DEBUG_STREAM()<<P[i]->getExtra().concavity_bpc<<endl;
//    }
	...
	//DUDE_ROS_DEBUG_STREAM()<<"total hole featured point number is "<<this->holeFeatPnts.size()<<endl;
...
void c_dude::collectConvexFeatPnts(c_polygon& P,list<ply_vertex*>& boundFeatPnts)
{
	...
	for(c_polygon::iterator pit=P.begin();pit!=P.end();pit++)
	{
		...
		if(m_bpcs.size() == 0 && totalPlySize != 1){
			...
			//DUDE_ROS_DEBUG_STREAM()<<"found boundaryFeatPnts number "<<boundFeatPnts.size()<<endl;
			//DUDE_ROS_DEBUG_STREAM()<<"discoverd!"<<endl;
			return;
		}
		...
		{
			//DUDE_ROS_DEBUG_STREAM()<<"find a convex ply"<<endl;
...
#if DEBUG
	DUDE_ROS_DEBUG_STREAM()<<"- Convex Hull:";
	for(IT i=hull.begin();i!=hull.end();i++) DUDE_ROS_DEBUG_STREAM()<<(*i)->getVID()<<", ";
	DUDE_ROS_DEBUG_STREAM()<<endl;
#endif
...
#if DEBUG
		DUDE_ROS_DEBUG_STREAM()<<"Bridge ("<<(*i)->getVID()<<","<<(*j)->getVID()<<")"<<endl;
#endif
...
//    {
//#if DEBUG
//    	DUDE_ROS_DEBUG_STREAM()<<"an abnormal diagonal"<<endl;
//#endif
...
bool c_dude::is_bridge_breaking(const ply_vertex * v, const ply_vertex * u)
{
    ...
    if(bpc1!=NULL && bpc2==NULL){
        //DUDE_ROS_DEBUG_STREAM()<<"bpc1="<<bpc1<<endl;
        //bpc1=bpc1->getRoot();
...
{
	const size_t cuts_before_resolution = m_cuts.size();
	set<c_diagonal*> cuts_to_drop;
	for(vector<c_diagonal>::iterator dit = m_cuts.begin(); dit != m_cuts.end(); ++dit)
	{
		c_diagonal& tdig = *dit;
		ply_vertex* cut_v1 = tdig.getV1();
		ply_vertex* cut_v2 = tdig.getV2();
		if(cut_v1 == NULL || cut_v2 == NULL)
		{
			cuts_to_drop.insert(&tdig);
			continue;
		}
		vector<Point2d> intersectionPositions;
		vector<pair<ply_vertex*, ply_vertex*> > intersectionEdges;
		double s[2], e[2];
		s[0] = cut_v1->getPos()[0]; s[1] = cut_v1->getPos()[1];
		e[0] = cut_v2->getPos()[0]; e[1] = cut_v2->getPos()[1];
			...
			for(c_polygon::iterator plyit = tmpPolygon.begin(); plyit != tmpPolygon.end(); ++plyit)
			{
				...
				//char intersectCode = SegSegInt(tdig.getV1()->getPos(), tdig.getV2()->getPos(), pcur->getPos(), pnext->getPos(), interPos);
					...
					{
						if(distance2Point(cut_v1->getPos(), interPos) >= SMALLZERO && distance2Point(cut_v2->getPos(), interPos) >= SMALLZERO)
						{
		...
		if(intersectionEdges.size() != intersectionPositions.size() || intersectionEdges.size() == 0)
		{
			cuts_to_drop.insert(&tdig);
			continue;
		}
		...
		{
			Point2d& tp = *vit;
			float distS = distance2Point(cut_v1->getPos(), tp);
			float distE = distance2Point(cut_v2->getPos(), tp);
			if(distS < sfarDist){
				sfarDist = distS;
				svit = vit;
				svitEdge = edgeit;
		...
		double sc[2], ec[2];
		sc[0] = (sInter[0] + cut_v1->getPos()[0]) / 2;
		sc[1] = (sInter[1] + cut_v1->getPos()[1]) / 2;
		ec[0] = (eInter[0] + cut_v2->getPos()[0]) / 2;
		ec[1] = (eInter[1] + cut_v2->getPos()[1]) / 2;
		...
		if(sOtherEdge.first == NULL || sOtherEdge.second == NULL ||
		   eOtherEdge.first == NULL || eOtherEdge.second == NULL)
		{
			cuts_to_drop.insert(&tdig);
			continue;
		}
		...
		if( distance2Point(cut_v1->getPos(), sInterOtherV->getPos())  > distance2Point(cut_v1->getPos(), sOtherEdge.second->getPos()) )
			sInterOtherV = sOtherEdge.second;
		if( distance2Point(cut_v2->getPos(), eInterOtherV->getPos())  > distance2Point(cut_v2->getPos(), eOtherEdge.second->getPos()) )
		...
		if(sInterClosed && eInterClosed){
			if(sfarDist > efarDist)
				tdig.setEndPoints(cut_v1, sInterOtherV);
			else
				tdig.setEndPoints(eInterOtherV, cut_v2);
		}
		...
		{
			DUDE_ROS_WARN_STREAM()<<"error in resolving intersected cuts... dropping cut"<<endl;
			cuts_to_drop.insert(&tdig);
		}
...
//		++dit;
	...
	{
		c_diagonal& tdig = *dit;
		if(cuts_to_drop.find(&tdig) != cuts_to_drop.end())
		{
			dit = m_cuts.erase(dit);
			continue;
		}
		ply_vertex* cut_v1 = tdig.getV1();
		ply_vertex* cut_v2 = tdig.getV2();
		if(cut_v1 == NULL || cut_v2 == NULL || cut_v1->getNext() == NULL || cut_v2->getNext() == NULL)
		{
			dit = m_cuts.erase(dit);
			continue;
		}
		if(cut_v1->getNext() == cut_v2 || cut_v2->getNext() == cut_v1)
		{
			dit = m_cuts.erase(dit);
			continue;
		}
		...
		++cdit;
			...
			for(; cdit != m_cuts.end(); ++cdit)
			{
				...
				ply_vertex* other_v1 = otherDiag.getV1();
				ply_vertex* other_v2 = otherDiag.getV2();
				if(other_v1 == NULL || other_v2 == NULL)
					continue;
				if((cut_v1 == other_v2 && cut_v2==other_v1) || (cut_v1 == other_v1 && cut_v2==other_v2))
				{
					dit = m_cuts.erase(dit);
					isRedudant = true;
					break;
				}
				if(checkCutSkinny(cut_v1, cut_v2, m_tau))
				{
					dit = m_cuts.erase(dit);
					isRedudant = true;
				break;
	...
	if(cuts_before_resolution != m_cuts.size() || !cuts_to_drop.empty())
	{
		DUDE_ROS_DEBUG_STREAM()<<"resolved cuts before="
			<<cuts_before_resolution
			<<" marked_drop="<<cuts_to_drop.size()
			<<" after="<<m_cuts.size()<<endl;
	}
}
