...
namespace mathtool{
    ...
    */
    void prompt(char *s)
    {
      DUDE_ROS_DEBUG_STREAM() << s << " ";
    }

    void message(char *s1, char *s2, char *s3)
    {
      DUDE_ROS_DEBUG_STREAM() << s1;
      if(s2 != NULL && strlen(s2) > 0)
        DUDE_ROS_DEBUG_STREAM() << " " << s2;
      if(s3 != NULL && strlen(s3) > 0)
        DUDE_ROS_DEBUG_STREAM() << " " << s3;
      DUDE_ROS_DEBUG_STREAM() << endl;
    }

    void status(char *s1, char *s2, char *s3)
    {
      DUDE_ROS_DEBUG_STREAM() << "Status: ";
      message(s1, s2, s3);
    }

    void error(char *s1, char *s2, char *s3)
    {
      DUDE_ROS_WARN_STREAM() << "Error: ";
      DUDE_ROS_WARN_STREAM() << s1;
      if(s2 != NULL && strlen(s2) > 0)
        DUDE_ROS_WARN_STREAM() << " " << s2;
      if(s3 != NULL && strlen(s3) > 0)
        DUDE_ROS_WARN_STREAM() << " " << s3;
      DUDE_ROS_WARN_STREAM() << endl;
    }
